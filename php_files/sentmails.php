...
?>
	...
	<?php
			...
			else if($_SESSION["logged"]=="user")
			{
				...
				$uid = $_SESSION["user"];
				$qry = "select * from mails where `from` = $uid order by time desc";
				$res = mysql_query($qry) or die(mysql_error());
				if(mysql_num_rows($res)>0)
				{	echo "<center><table><tr><td><center>Mailed To</td><td><center>Message</td><td><center>Time</td><td><center>Delete</td><td><center>Reply</center></td></tr>";
					while($row = mysql_fetch_array($res))
					{
						$mid = $row["mid"];
						$to = $row["to"];
						$qry1 = "select name from users where uid = $to ";
						$res1 = mysql_query($qry1) or die(mysql_error());
						$row1= mysql_fetch_array($res1);
						echo "<tr>";
						echo "<td style='padding:20px;'><a href='profile.php?uid=$to'>".$row1["name"]."</a></td>";
						...
						<br><input type='text' hidden name='to' value=$to>
